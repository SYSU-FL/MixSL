...
class Client(object):
    ...
    def get_labels_factors(self, a, b, lam):
        lam_y = 0.
        if (self.num_per_cls[a] / self.num_per_cls[b] >= 2) and (lam < 0.5):
            lam_y = 0.
        elif (self.num_per_cls[a] / self.num_per_cls[b] <= 1) and ((1 - lam) < 0.5):
            lam_y = 1.
        else:
            lam_y = lam
        return lam_y
...
if __name__ == "__main__":
    ...
    TAG = 'multi-mixsl-mixsum' + str(args.mix_num) + '-' + data_name + '-' + args.name
