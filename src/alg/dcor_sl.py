...
from utils import save_logs, save_cifar100_logs, setup_seed
from function import random_assign, differential_privacy, distcorr,compute_dcor, assgin_task
...
class Client(object):
    ...
    def train(self, weight=None):
        ...
        for batch_idx, (images, labels) in enumerate(self.loader):
            ...
            loss, grad = self.server.train(images, outputs, labels)
...
class Server(object):
    def __init__(self, model, args):
        ...
        self.optimizer = torch.optim.SGD(self.model.parameters(), lr=self.args.lr, momentum=self.args.momentum, weight_decay=1e-5)
        self.inputs = []
    def train(self, images, inputs, labels):
        input_temp = inputs.clone().detach()
        input_temp.requires_grad_()
        self.model.train()
        self.model.zero_grad()
        outputs = self.model(input_temp)
        loss = self.criterion(images, input_temp, outputs, labels)
        loss.backward()
...
if __name__ == "__main__":
    ...
    TAG = 'dcor-sl-' + prefix_tag + '-' +  args.name
